...
% Demonstrates how dynamic adaptation maximizes multiplexing efficiency across all SNR regimes.
...
snr_db_vec = -4:2:24;
num_frames = 200;
frame_syms = 256; % Symbols per frame / packet
...
mode_selection_prob = zeros(length(snr_db_vec), 1); % Prob of selecting Multiplexing mode
...
for s_idx = 1:length(snr_db_vec)
    ...
    for f = 1:num_frames
        % 1. Generate Correlated Channel realization
        [H, ~, ~] = generate_correlated_channel(2, 2, rho, rho, 1);
        
        % -------------------------------------------------------------
        % Path A: Fixed Diversity Mode (Alamouti STBC)
        % Transmit 1 stream (frame_syms symbols, length frame_syms*k bits)
        % -------------------------------------------------------------
        ...
        % -------------------------------------------------------------
        % Path B: Fixed Spatial Multiplexing (2 streams, MMSE detector)
        % Transmits 2*frame_syms symbols, length 2*frame_syms*k bits
        % -------------------------------------------------------------
        ...
        % -------------------------------------------------------------
        % Path C: Adaptive Mode Controller
        % -------------------------------------------------------------
        [mode_decision, ~, ~] = adaptive_mimo_controller(H, snr_db, mod_order);
        if mode_decision == 1
            sm_mode_count = sm_mode_count + 1;
            if is_sm_success
                adapt_frame_success = adapt_frame_success + 2; % Delivers 2 streams
            end
        else
            if is_div_success
                adapt_frame_success = adapt_frame_success + 1; % Delivers 1 stream
            end
        end
    end
    
    % Goodput = Nominal Data Rate * Packet Success Rate (bps/Hz equivalent)
    ...
    mode_selection_prob(s_idx) = sm_mode_count / num_frames;
end
